    ...
    <div className='photosdiv'>
      {mediaData.map((item, index) => (
        ...
        {item.imgurl1.map((imgUrl, imgIndex) => ( 
            <img style={{width:"200px", height:"300px"}} key={`img-${index}-${imgIndex}`} src={imgUrl} alt={`Image ${imgIndex}`} />
              ))}
            </div>
        </div>
          ...
          {item.vidurl1.length > 0 && (
              ...
              {item.vidurl1.map((vidUrl, vidIndex) => (
                <div>
                    <video style={{width:"200px", height:"300px"}} key={`vid-${index}-${vidIndex}`} src={vidUrl} controls />
                </div>
              ))}
            </div>
          )}        
