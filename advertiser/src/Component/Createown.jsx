...
export const Createown = () => {
  ...
  useEffect(() => {
    ...
    // Fetch videos
    const videoApiUrl =
      "https://api.pexels.com/videos/search?query=nature&per_page=30";
  ...
  const handleImageClick = (photo) => {
    ...
    if (isAlreadySelected) {
      ...
      const updatedSelection = selectedPhotos.filter(
        (selectedPhoto) => selectedPhoto.id !== photo.id
      );
  ...
  const handleVideoClick = (video) => {
    ...
    if (isAlreadySelected) {
      ...
      const updatedSelection = selectedVideos.filter(
        (selectedVideo) => selectedVideo.id !== video.id
      );
  ...
  const handleNextClick = async () => {
    ...
    const formattedMediaData = [{ Allinoneid: 123 }];
    ...
    try {
      const response = await fetch(
        "https://lonely-cow-life-jacket.cyclic.app/imgandvid",
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="App">
      ...
      <button
        ...
        disabled={selectedPhotos.length + selectedVideos.length < 1}
      >
        Next
      </button>
